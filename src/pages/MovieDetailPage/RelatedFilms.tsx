...
export function RelatedFilms({ relatedFilms }: IRelatedFilmsProps) {
  ...
  const PrevArrow = (props: any) => (
    <button className={`slider-button prev`} onClick={props.onClick}>
    </button>
  );

  const NextArrow = (props: any) => (
    <button className={`slider-button next`} onClick={props.onClick}>
    </button>
  );
        ...
        {relatedFilms.map((film) => (
          ...
          >
            <div className="relative min-w-[150px]">
              <img src={film.posterUrlPreview} className="min-w-[150px] min-h-[225px]" alt="poster" />
